...
public class NamespaceOpenApiService extends AbstractOpenApiService
  ...
  public OpenNamespaceDTO getNamespace(String appId, String env, String clusterName,
    ...
    try (CloseableHttpResponse response = get(pathBuilder)) {
      ...
      throw new RuntimeException(
          String.format("Get namespace for appId: %s, cluster: %s, namespace: %s in env: %s failed",
  ...
  public List<OpenNamespaceDTO> getNamespaces(String appId, String env, String clusterName,
    ...
    OpenApiPathBuilder pathBuilder = OpenApiPathBuilder.newBuilder().envsPathVal(env)
        .appsPathVal(appId).clustersPathVal(clusterName).customResource("namespaces");
    ...
    try (CloseableHttpResponse response = get(pathBuilder)) {
      ...
      throw new RuntimeException(
          String.format("Get namespaces for appId: %s, cluster: %s in env: %s failed", appId,
              clusterName, env),
          ex);
    }
  }
  ...
  public OpenAppNamespaceDTO createAppNamespace(OpenAppNamespaceDTO appNamespaceDTO) {
    ...
    OpenApiPathBuilder pathBuilder = OpenApiPathBuilder.newBuilder()
        .appsPathVal(appNamespaceDTO.getAppId()).customResource("appnamespaces");
    ...
    try (CloseableHttpResponse response = post(pathBuilder, appNamespaceDTO)) {
      ...
      throw new RuntimeException(
          String.format("Create app namespace: %s for appId: %s, format: %s failed",
  ...
  public OpenNamespaceLockDTO getNamespaceLock(String appId, String env, String clusterName,
    ...
    try (CloseableHttpResponse response = get(pathBuilder)) {
      ...
      throw new RuntimeException(String.format(
          "Get namespace lock for appId: %s, cluster: %s, namespace: %s in env: %s failed", appId,
