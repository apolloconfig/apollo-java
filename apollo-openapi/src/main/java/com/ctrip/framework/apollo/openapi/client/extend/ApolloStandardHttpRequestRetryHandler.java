...
public class ApolloStandardHttpRequestRetryHandler extends DefaultHttpRequestRetryHandler {
  ...
  public ApolloStandardHttpRequestRetryHandler(int retryCount, IdempotentHttpMethod[] httpMethods) {
    ...
    if (httpMethods == null || httpMethods.length == 0) {
      ...
      for (IdempotentHttpMethod idempotentHttpMethod : IdempotentHttpMethod.safe()) {
        idempotentMethods.put(idempotentHttpMethod.name(), Boolean.TRUE);
      }
      return;
    }
    for (IdempotentHttpMethod httpMethod : httpMethods) {
      ...
      idempotentMethods.put(httpMethod.name(), Boolean.TRUE);
    }
  }
  ...
  protected boolean handleAsIdempotent(final HttpRequest request) {
    ...
    return idempotentMethods.getOrDefault(method, Boolean.FALSE);
  }
}
