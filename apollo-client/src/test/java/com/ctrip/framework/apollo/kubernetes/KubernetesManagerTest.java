...
public class KubernetesManagerTest {
  ...
  public void testCreateConfigMapSuccess() throws Exception {
    ...
    V1ConfigMap configMap =
        new V1ConfigMap().metadata(new V1ObjectMeta().name(name).namespace(namespace)).data(data);

    when(coreV1Api.createNamespacedConfigMap(eq(namespace), eq(configMap), isNull(), isNull(),
        isNull(), isNull())).thenReturn(configMap);

    // act
    String result = kubernetesManager.createConfigMap(namespace, name, data);

    // assert
    verify(coreV1Api, times(1)).createNamespacedConfigMap(eq(namespace), any(V1ConfigMap.class),
  ...
  public void testUpdateConfigMapSuccess() throws Exception {
    ...
    V1Pod pod = new V1Pod().metadata(new V1ObjectMeta().name("localPodName")
        .creationTimestamp(OffsetDateTime.now()).labels(Collections.singletonMap("app", "app")));
    ...
    when(coreV1Api.listNamespacedPod(namespace, null, null, null, null, "app=app", null, null, null,
        null, null)).thenReturn(v1PodList);
    when(coreV1Api.readNamespacedConfigMap(name, namespace, null)).thenReturn(configMap);
    when(coreV1Api.replaceNamespacedConfigMap(name, namespace, configMap, null, null, null, null))
    ...
    // assert
    assertTrue(success);
    Mockito.verify(coreV1Api, Mockito.times(1)).listNamespacedPod(namespace, null, null, null,
        "status.phase=Running", "app=app", 100, null, null, null, null);
  }
  ...
  public void testCheckConfigMapExistWhenConfigMapDoesNotExist() throws Exception {
    ...
    String namespace = "default";
    String name = "testConfigMap";
    doThrow(new ApiException("ConfigMap not exist")).when(coreV1Api).readNamespacedConfigMap(name,
        namespace, null);
