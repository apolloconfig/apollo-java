...
import com.ctrip.framework.apollo.monitor.internal.event.ApolloConfigMetricsEventFactory;
...
public class ApolloClientMonitorMessageProducer implements MessageProducer {
    ...
    private void publishErrorEvent(String tag, Throwable cause) {
        ApolloConfigMetricsEventFactory.getInstance().createEvent(tag)
            .withTag(tag)
    ...
    private void publishConfigChangeEvent(String name) {
        ApolloConfigMetricsEventFactory.getInstance()
    ...
    private void publishMetaServiceEvent() {
        ApolloConfigMetricsEventFactory.getInstance().createEvent(META_FRESH)
    ...
    private void publishConfigServiceEvent(String name) {
        ApolloConfigMetricsEventFactory.getInstance().createEvent(CONFIG_SERVICE_URL)
            .withTag(TAG_BOOTSTRAP)
    ...
    private void publishClientVersionEvent(String name) {
        ApolloConfigMetricsEventFactory.getInstance().createEvent(VERSION)
            .withTag(TAG_BOOTSTRAP)
    ...
    private void publishNamespaceTimeoutEvent(String name) {
        ApolloConfigMetricsEventFactory.getInstance().createEvent(APOLLO_CLIENT_NAMESPACE_TIMEOUT)
            .putAttachment(NAMESPACE, name)
            .withTag(TAG_NAMESPACE)
            .publish();
    }
    ...
    private void publishNamespaceNotFoundEvent(String name) {
        ApolloConfigMetricsEventFactory.getInstance().createEvent(APOLLO_CLIENT_NAMESPACE_NOT_FOUND)
            .withTag(TAG_NAMESPACE)
            .putAttachment(NAMESPACE, name)
            .publish();
    }
    ...
    private void handleClientConfigEvent(String type, String name) {
        ...
        ApolloConfigMetricsEventFactory.getInstance().createEvent(NAMESPACE_RELEASE_KEY)
            .withTag(TAG_NAMESPACE)
    ...
    public void logMetricsForCount(String name) {
        ...
        if (split.length == 2 && APOLLO_CLIENT_NAMESPACE_USAGE.equals(split[0])) {
            ApolloConfigMetricsEventFactory.getInstance().createEvent(APOLLO_CLIENT_NAMESPACE_USAGE)
