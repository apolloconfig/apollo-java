 ...
 * @author Rawven
 */
public class DefaultApolloClientBootstrapArgsApi extends AbstractMetricsCollector implements
  ...
  public DefaultApolloClientBootstrapArgsApi(ConfigUtil configUtil) {
    ...
    bootstrapArgs.put(APOLLO_CLIENT_MONITOR_EXCEPTION_SAVE_SIZE,
        configUtil.getMonitorExceptionSaveSize());
    bootstrapArgs.put(APOLLO_CLIENT_MONITOR_METRICS_EVENT_POOL_SIZE,
        configUtil.getMonitorMetricsEventPoolSize());
  ...
  public void collect0(ApolloConfigMetricsEvent event) {
    switch (event.getName()) {
      case VERSION:
        bootstrapArgs.put(VERSION, event.getAttachmentValue(VERSION));
        break;
      case META_FRESH:
        bootstrapArgs.put(META_FRESH, event.getAttachmentValue(META_FRESH));
        break;
      case CONFIG_SERVICE_URL:
        bootstrapArgs.put(CONFIG_SERVICE_URL, event.getAttachmentValue(CONFIG_SERVICE_URL));
        break;
      default:
        logger.warn("Unhandled event name: {}", event.getName());
        break;
    }
  }
  ...
  public String getStartupParams(String key) {
    return bootstrapArgs.getOrDefault(key, "").toString();
  }
  ...
  public int getClientMonitorExceptionSaveSize() {
    return (int) bootstrapArgs.get(APOLLO_CLIENT_MONITOR_EXCEPTION_SAVE_SIZE);
  }

  @Override
  public int getClientMonitorMetricsEventPoolSize() {
    return (int) bootstrapArgs.get(APOLLO_CLIENT_MONITOR_METRICS_EVENT_POOL_SIZE);
  }
