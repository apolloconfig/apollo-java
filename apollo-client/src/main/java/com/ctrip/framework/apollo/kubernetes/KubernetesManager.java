...
public class KubernetesManager {
  ...
  @VisibleForTesting
  public KubernetesManager(CoreV1Api coreV1Api, String localPodName,
  ...
  public String createConfigMap(String k8sNamespace, String name, Map<String, String> data) {
    if (StringUtils.isEmpty(k8sNamespace) || StringUtils.isEmpty(name)) {
      logger.error(
          "create configmap failed due to null or empty parameter: k8sNamespace={}, name={}",
          k8sNamespace, name);
      return null;
    }
  ...
  public String getValueFromConfigMap(String k8sNamespace, String name, String key) {
    if (StringUtils.isEmpty(k8sNamespace) || StringUtils.isEmpty(name)
      ...
      logger.error("Parameters can not be null or empty: k8sNamespace={}, name={}", k8sNamespace,
          name);
      return null;
    }
    try {
      ...
      if (!Objects.requireNonNull(configMap.getData()).containsKey(key)) {
        logger.error("Specified key not found in ConfigMap: {}, k8sNamespace: {}, name: {}", name,
            k8sNamespace, name);
      }
  ...
  public boolean updateConfigMap(String k8sNamespace, String name, Map<String, String> data)
    ...
    while (retryCount < maxRetries) {
      try {
        ...
        if (e.getCode() == 409) {
          ...
          try {
            // Scramble the time, so that different machines in the distributed retry time is
            // different
    ...
    String errorMessage =
        String.format("Failed to update ConfigMap after %d retries: k8sNamespace=%s, name=%s",
            maxRetries, k8sNamespace, name);
  ...
  public boolean checkConfigMapExist(String k8sNamespace, String configMapName) {
    if (StringUtils.isEmpty(k8sNamespace) || StringUtils.isEmpty(configMapName)) {
      logger.error("Parameters can not be null or empty: k8sNamespace={}, configMapName={}",
          k8sNamespace, configMapName);
      return false;
    }
    try {
  ...
  private boolean isWritePod(String k8sNamespace) {
    try {
      ...
      V1PodList v1PodList = coreV1Api.listNamespacedPod(k8sNamespace, null, null, null,
          RUNNING_POD_FIELD_SELECTOR, labelSelector, MAX_SEARCH_NUM, null, null, null, null);

      return v1PodList.getItems().stream().map(V1Pod::getMetadata).filter(Objects::nonNull)
          ...
          .map(V1ObjectMeta::getName).limit(propertyKubernetesMaxWritePods)
